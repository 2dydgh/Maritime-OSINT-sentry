...
def test_exact_evidence_and_server_numbers(investigation):
    ...
    assert result["facts"]["data"]["approval_check"]["dcpa_nm"] < 0.01
...
def test_forged_or_missing_evidence_is_rejected(investigation, case):
    ...
    r = ctx.record
    if case == "unknown":
        d.evidence_ids[0] = "invented"
    elif case == "duplicate":
        d.evidence_ids[1] = d.evidence_ids[0]
    elif case == "missing_graph":
        d.evidence_ids[1] = ctx.inspect()["id"]
    else:
        e = r["evidence"][d.evidence_ids[0]]
        if case == "other_pair":
            e["pair"] = [111111111, 222222222]
        else:
            e["proposal_id"] = "another-incident"
...
def test_old_inspection_or_mid_run_change_cannot_pass(investigation):
    ...
    d = draft(ctx)
    r = ctx.record
    r["evidence"][d.evidence_ids[0]]["data"]["checked_at"] -= 61
    ctx.repo.update(ctx.rid, evidence=r["evidence"])
...
def test_duplicate_start_cancel_and_restart_recovery(investigation):
    ...
    r, created = ctx.repo.create(pid, 180, "test")
    assert created
...
def test_approval_rejects_expired_or_changed_result(investigation, case):
    ...
    result = ctx.validate(draft(ctx))
    if case == "expired":
        result["validated_at"] -= 61
    ctx.repo.update(ctx.rid, status="completed", result=result)
    if case == "changed":
        lookup(PAIR)[1]["cog"] = 90
    if case == "changed":
        result = ctx.repo.store.decide(
            ctx.record["proposal_id"], "approved", "test", "tab", risks, lookup, investigation_id=ctx.rid
        )
        assert not result["execute"]
    else:
        with pytest.raises(ValueError):
            ctx.repo.store.decide(
                ctx.record["proposal_id"],
                "approved",
                "test",
                "tab",
                risks,
                lookup,
                investigation_id="missing" if case == "unknown" else ctx.rid,
            )
...
def function_model(action="focus_tracking", reason="risk_persists", extra=False):
    async def respond(messages, info):
        ...
        if "inspect_current" not in names:
            return ModelResponse(parts=[ToolCallPart("inspect_current", {})])
        if extra and "recalculate_cpa" not in names:
            return ModelResponse(parts=[ToolCallPart("recalculate_cpa", {})])
        if "query_knowledge" not in names:
            return ModelResponse(parts=[ToolCallPart("query_knowledge", {})])
        ids = [p.content["id"] for p in returns if p.tool_name in ("inspect_current", "query_knowledge")]
        return ModelResponse(
            parts=[ToolCallPart(info.output_tools[0].name, dict(action=action, reason=reason, evidence_ids=ids))]
        )
...
async def test_timeout_and_cancel_stop_model(investigation, cancel):
    ctx, _, _ = investigation
    if not cancel:
        ctx.repo.update(ctx.rid, deadline=time.time() + 0.1)
    ...
    with TestClient(app) as client:
        ...
        assert (
            client.post(
                ...
                json=dict(outcome="approved", reason="test", client_id="tab", investigation_id=ctx.rid),
            ).status_code
            == 409
        )
...
def test_reopened_result_reflects_case_decision_without_rewriting_evidence(investigation):
    ...
    ctx.repo.store.decide(
        original["proposal_id"], "approved", "조사 근거 확인", "tab", risks, lookup, investigation_id=ctx.rid
    )
...
def test_expired_investigation_can_be_dismissed_but_not_approved(investigation):
    ...
    decision = ctx.repo.store.decide(
        record["proposal_id"], "dismissed", "과거 근거를 검토하고 기각", "tab", risks, lookup, investigation_id=ctx.rid
    )
...
def test_changed_stale_reason_and_expiry_do_not_restart_same_recommendation(investigation):
    ...
    with pytest.raises(ValueError):
        ctx.validate(a.Recommendation(action="focus_tracking", reason="risk_persists", evidence_ids=d.evidence_ids))
            ...
            parts=[
                ToolCallPart(
                    ...
                    dict(
                        ...
                        evidence_ids=[r.content["id"] for r in returns],
                    ),
                )
            ]
        )
