...
def _fake_weather(cells_with_values):
    return {"cells": [{"lat": lat, "lng": lng, **vals} for (lat, lng), vals in cells_with_values.items()]}
...
def test_compute_cells_returns_only_cells_above_threshold():
    ...
    weather = _fake_weather(
        {
            (lat, lng): {"wave_height": 0, "wind_speed": 0, "visibility": 20000, "wind_direction": 0, "wave_period": 0}
            for (lat, lng) in cells
        }
    )
...
def test_compute_cells_high_wave_produces_cell():
    ...
    target = cells[0]
    weather = _fake_weather(
        {target: {"wave_height": 5.0, "wind_speed": 5, "visibility": 20000, "wind_direction": 0, "wave_period": 7}}
    )
    result = korea_hex_grid.compute_cells(weather, vessels=[], features=[])
...
def test_top_cause_picks_dominant_factor():
    ...
    raw_values = {"wave_raw": 1.4, "wind_raw": 50.0, "vis_raw": 8.0, "traffic_n": 0, "static_names": []}
...
def test_top_cause_static_takes_name():
    ...
    raw_values = {"wave_raw": 0, "wind_raw": 0, "vis_raw": 10, "traffic_n": 0, "static_names": ["울돌목 협수로"]}
