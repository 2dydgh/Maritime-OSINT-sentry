...
def vessels():
    return [
        dict(mmsi=A, name="A", lat=0, lng=0, sog=12, cog=90, _updated=NOW),
...
def test_same_initial_conditions_and_neighbor_tradeoff():
    snap = s.capture([A, B], vessels(), NOW)
...
def test_saved_api_replays_without_live_data(tmp_path, monkeypatch):
    ...
    response = client.post(
        prefix + "/runs", json={"snapshot_id": snap["id"], "target": A, "speed": 12, "course": 0, "minutes": 15}
    )
    ...
    assert (
        client.post(
            prefix + "/runs", json={"snapshot_id": snap["id"], "target": C, "speed": 12, "course": 0}
        ).status_code
        == 422
    )
...
def test_ramped_speed_distance_and_course_wrap():
    ...
    assert points[-1]["y_nm"] == pytest.approx(0.1)  # mean 6 kt over 1 minute
