...
def setup(tmp_path):
    ...
    pid = store.list()[0]["id"]
    snapshot = s.capture(list(PAIR), lookup(PAIR), NOW)
...
def test_review_binding_idempotency_and_approval_evidence(setup):
    ...
    assert (
        len(store.review_scenario(pid, run, "reference", "주변 확인", "tab", "review1", NOW + 2)["scenario_reviews"])
        == 1
    )
    with pytest.raises(ValueError):
        store.review_scenario(pid, run, "rejected", "주변 확인", "tab", "review1", NOW + 2)
    wrong = copy.deepcopy(run)
    wrong["snapshot"]["proposal_id"] = "other"
    with pytest.raises(ValueError):
        store.review_scenario(pid, wrong, "reference", "이유", "tab", "review2", NOW + 2)
...
def test_prediction_errors_use_each_reception_time_not_poll_time(setup):
    ...
    assert result["tracks"][str(PAIR[0])][0]["errors_nm"]["alternative"] > 0.1
