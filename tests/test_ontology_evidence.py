...
def saved(tmp_path):
    ...
    repo = s.Repository(tmp_path / "scenarios.db")
    risks, lookup = data()
    # Production trigger inputs usually lack reception times.
    for risk in risks["distance"]:
        risk["ship_a"] = dict(risk["ship_a"])
        risk["ship_a"].pop("_updated")
        risk["ship_b"] = dict(risk["ship_b"])
        risk["ship_b"].pop("_updated")
    store.refresh(risks, lookup, NOW)
    pid = store.list()[0]["id"]
    snap = s.capture(list(PAIR), lookup(PAIR), NOW)
    snap["proposal_id"] = pid
    first = s.compare(snap, PAIR[0], 6, 0, 15, 60, 120)
    repo.save("run", first)
    ...
    second = s.compare(snap, PAIR[0], 8, 10, 15)
    repo.save("run", second)
    store.review_scenario(pid, second, "deferred", "승인 후 추가 검토", "tab", "after", NOW + 4)
    for offset in (5, 10):
        for vessel in lookup(PAIR):
            vessel["_updated"] = NOW + offset
...
def test_real_mapping_queries_and_time_frozen_basis(saved):
    ...
    after_review = next(
        n
        for n in basis["graph"]["nodes"]
        if n["type"] == "Review" and any(p["value"] == "승인 후 추가 검토" for p in n["properties"])
    )
...
def test_receipt_only_from_execution_event_not_review_status(saved):
    ...
    raw["proposal"]["events"] = [
        e
        for e in raw["proposal"]["events"]
        if e.get("kind") != "execution_receipt" and e.get("reason") != "map_tracking_started"
    ]
...
def test_followup_fresh_stale_ended_and_no_judgment(saved):
    ...
    raw["proposal"]["decision"] = None
    raw["proposal"]["execution"] = None
...
def test_shacl_rejects_bad_observation_and_prediction_confusion(saved):
    ...
    view.g.set((obs, M.latitude, Literal(999.0, datatype=XSD.double)))
...
def test_read_only_api_source_fingerprint_and_no_arbitrary_queries(saved, monkeypatch):
    store, repo, pid, _, _ = saved
    monkeypatch.setattr(s.config, "WATCH_DB", str(store.path))
    monkeypatch.setattr(s.config, "SCENARIO_DB", str(repo.path))
    app = FastAPI()
