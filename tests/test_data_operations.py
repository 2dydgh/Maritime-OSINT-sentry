...
def test_backup_preserves_live_wal_and_does_not_overwrite(tmp_path):
    ...
    db.execute('PRAGMA journal_mode=WAL');db.execute('CREATE TABLE sample(id INTEGER)')
    db.execute('INSERT INTO sample VALUES(7)');db.commit()
    ...
    with pytest.raises(FileExistsError):backup(tmp_path/'copy',paths)
