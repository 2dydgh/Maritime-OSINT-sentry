...
def decide(pid: str, body: Decision):
    try:
        ...
        return watch_officer.get_store().decide(
            pid, body.outcome, body.reason, body.client_id, risks, lookup, investigation_id=body.investigation_id
        )
    except KeyError:
...
def review(pid: str, body: ScenarioReview):
    try:
        ...
        return watch_officer.get_store().review_scenario(
            pid, run, body.outcome, body.reason, body.client_id, body.review_id
        )
    except KeyError:
