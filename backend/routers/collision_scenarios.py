...
def capture(body:Pair):
    try:
        if body.proposal_id:
            ...
            with store.transaction() as db: proposal=store.get(db,body.proposal_id)
        ...
        if body.proposal_id:
            snapshot['proposal_context']={'revision':proposal['revision'],'status':proposal['status'],'trigger':proposal['trigger'],'rule':proposal['rule']}
        return service.Repository().save('snapshot',snapshot)
    except KeyError:raise HTTPException(404,'제안을 찾을 수 없습니다')
...
def compare(body:Comparison):
    ...
    try:
        ...
        return repo.save('run',service.compare(snapshot,body.target,body.speed,body.course,body.minutes,body.turn_seconds,body.speed_seconds))
    except KeyError:raise HTTPException(404,'스냅샷을 찾을 수 없습니다')
    except ValueError as e:raise HTTPException(422,str(e))

@router.get('/runs')
def recent():return {'runs':service.Repository().recent()}

@router.get('/runs/{rid}')
def get_run(rid:str):
    try:return service.Repository().get('run',rid)
    except KeyError:raise HTTPException(404,'비교 기록을 찾을 수 없습니다')
