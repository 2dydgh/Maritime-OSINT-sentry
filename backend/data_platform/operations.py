...
def sources():
    ...
    return {
        "watch": Path(config.WATCH_DB),
        "scenarios": Path(config.SCENARIO_DB),
        "journal": Path(config.DATA_PLATFORM_DB),
    }
...
def backup(directory, paths=None):
    ...
    manifest = {
        ...
        "files": {},
        "note": "DB별 일관된 순차 백업이며 전체 서비스의 단일 시점 스냅샷은 아닙니다. 원본을 삭제하지 않습니다.",
    }
    for name, path in (paths or sources()).items():
        path = Path(path)
        if not path.exists():
        ...
        try:
            ...
            if dst.execute("PRAGMA integrity_check").fetchone()[0] != "ok":
                raise RuntimeError("Backup integrity check failed: " + name)
        finally:
            src.close()
            dst.close()
    ...
    if "watch" in manifest["files"]:
        watch = readonly(directory / "watch.sqlite3")
        scenarios = readonly(directory / "scenarios.sqlite3") if "scenarios" in manifest["files"] else None
        try:
            scenario_ids = set()
            ...
            if watch.execute("SELECT 1 FROM sqlite_master WHERE type='table' AND name='investigations'").fetchone():
                for row in watch.execute("SELECT record FROM investigations"):
                    for evidence in json.loads(row[0]).get("evidence", {}).values():
                        ...
                        if run_id:
                            scenario_ids.add(run_id)
            for run_id in scenario_ids:
                if (
                    scenarios is None
                    or not scenarios.execute("SELECT 1 FROM records WHERE kind='run' AND id=?", (run_id,)).fetchone()
                ):
