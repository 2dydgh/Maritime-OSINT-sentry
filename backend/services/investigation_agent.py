...
TIMEOUT = 180
ACTION_LABELS = {
    "focus_tracking": "집중 추적 검토",
    "check_observations": "관측·분석 갱신 확인",
    "continue_monitoring": "기존 추적 상태 확인",
    "operator_review": "운용자 추가 검토",
    "no_new_action": "추가 추적 제안 보류",
}
REASONS = {
    "risk_persists": "최신 자료가 기존 집중 추적 제안 조건을 충족합니다.",
...
class Investigation:
    ...
    def current(self):
        ...
        return {
            ...
            "checked_at": now,
            "error": error,
            "approval_check": checked,
            "subjects": subjects,
            "candidate": candidate,
            "analysis_updated_at": risks["updated_at"],
        }
    ...
    def knowledge(self):
        def read():
            ...
            answer["graph"]["edges"] = [e for e in answer["graph"]["edges"] if e["from"] in ids and e["to"] in ids][
                :160
            ]
    ...
    def recalculate(self):
        def calculate():
            ...
            a, b = vessels
            tcpa, dcpa = w.collision_analyzer._compute_tcpa_dcpa(
                *(a[k] for k in ("lat", "lng", "sog", "cog")), *(b[k] for k in ("lat", "lng", "sog", "cog"))
            )
            return {
                "available": True,
                "method": "existing-cpa-calculation",
                "checked_at": state["checked_at"],
                "subjects": vessels,
                "dcpa_nm": dcpa if math.isfinite(dcpa) else None,
    ...
    def compare(self, target: Literal["first", "second"], change: Literal["slower", "port", "starboard"]):
        def calculate():
            completed = [
                v
                for v in self.record["evidence"].values()
                if v["tool"] == "compare_scenario" and v["data"].get("run_id")
            ]
            if len(completed) >= 2:
            ...
            snapshot.update(
                proposal_id=p["id"], proposal_context={k: p[k] for k in ("revision", "status", "trigger", "rule")}
            )
            vessel = snapshot["ships"][0 if target == "first" else 1]
            speed = vessel["sog"] * 0.8 if change == "slower" else vessel["sog"]
    ...
    def validate(self, draft, record=True):
        ...
        if allowed != observed_allowed:
            ...
            raise ValueError(
                "조사 중 필요한 조치가 변경됐습니다. 서버가 다시 확인한 근거 "
                + fresh["id"]
                + "와 기존 온톨로지 근거를 인용하세요. action/reason: "
                + str(allowed)
            )
        ...
        facts = (
            self.repo.event(self.rid, "validate_result", "completed", current)
            if record
        ...
        return {
            **draft.model_dump(),
            "action_label": ACTION_LABELS[draft.action],
            "explanation": REASONS[draft.reason],
            "facts": facts,
...
def build_agent(model):
    ...
    async def latest_evidence(ctx: RunContext[Investigation]) -> str:
        ...
        return (
            "현재 확보한 근거로 가능한 결과: "
            + str(ready)
            + (" . 추가 조회를 종료하고 final_result를 제출하세요." if ready["action"] != "focus_tracking" else "")
        )
    ...
    async def compare_scenario(
        ctx: RunContext[Investigation],
        target: Literal["first", "second"],
        change: Literal["slower", "port", "starboard"],
    ) -> dict:
...
async def execute(investigation, model=None):
    ...
    async def work():
        async with httpx.AsyncClient(timeout=60) as client:
            selected = model or OpenAIChatModel(
                MODEL,
                provider=OllamaProvider(base_url=config_llm.OLLAMA_BASE_URL.rstrip("/") + "/v1", http_client=client),
            )
            result = await build_agent(selected).run(
                "선택된 사건을 조사하고 근거를 갖춘 다음 조치를 제안하세요.",
                deps=investigation,
                usage_limits=UsageLimits(request_limit=8, tool_calls_limit=10, total_tokens_limit=24000),
            )
            usage = result.usage
            return investigation.validated, {
                k: getattr(usage, k) for k in ("input_tokens", "output_tokens", "requests", "tool_calls")
            }
    ...
    try:
        ...
        result = await asyncio.to_thread(
            investigation.validate, Recommendation(**{k: result[k] for k in ("action", "reason", "evidence_ids")})
        )
        repo.update(
            rid,
            status="completed",
            result=result,
            usage=usage,
            message="조사가 완료됐습니다. 근거와 권장 조치를 검토하세요.",
        )
    except asyncio.CancelledError:
        repo.update(rid, status="cancelled", message="조사가 취소됐습니다.")
    except TimeoutError:
        repo.update(
            rid, status="timed_out", message="조사 시간 한도를 넘었습니다. 저장된 확인 결과를 보고 다시 시도하세요."
        )
