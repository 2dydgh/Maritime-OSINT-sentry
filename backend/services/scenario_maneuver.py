...
def path(ship, speed, course, turn_seconds, speed_seconds, horizon, origin):
    ...
    if delta == -180:
        delta = 180
    def state(t):
        return (ship['sog']+(speed-ship['sog'])*min(1,t/speed_seconds) if speed_seconds else speed,
                (ship['cog']+delta*min(1,t/turn_seconds))%360 if turn_seconds else course)
    x,y=ship['x_nm'],ship['y_nm']
    points=[]
    for t in range(horizon+1):
        if t:
            v,c=state(t-.5);angle=math.radians(c)
            x+=v*math.sin(angle)/3600;y+=v*math.cos(angle)/3600
        v,c=state(t)
...
def closest_to_path(points, other, origin):
    ...
    for a,b in pairwise(points):
        ...
        if best is None or distance<best[0]:best=(distance,t+f,a,b,f)
    distance,t,a,b,f=best
    projected={k:a[k]+(b[k]-a[k])*f for k in ('x_nm','y_nm')}
    p=s.point(dict(other,**projected,sog=0),0,origin);p['t_s']=t
    return {'closest_distance_nm':distance,'closest_time_s':t,'tcpa_min':None,'dcpa_nm':None,
            'within_horizon':None,'metric_method':'piecewise-linear-1s','a':p,'b':s.point(other,t,origin)}


def apply(result, turn_seconds, speed_seconds):
    from backend.services import collision_scenarios as s
    snapshot=result['snapshot'];target=result['change']['mmsi'];origin=snapshot['origin']
    subject=next(v for v in snapshot['ships'] if v['mmsi']==target)
    points=path(subject,result['change']['sog'],result['change']['cog'],turn_seconds,speed_seconds,int(result['horizon_s']),origin)
    ...
    if snapshot['pair'][0]!=target:metric['a'],metric['b']=metric['b'],metric['a']
    alt['pair']=metric
    nearby=[]
    for neighbor in snapshot['ships'][2:]:
        for vessel in (subject,other):
            metric=closest_to_path(points,neighbor,origin) if vessel is subject else s.closest(vessel,neighbor,result['horizon_s'],origin)
            nearby.append({'subject':vessel['mmsi'],'other':neighbor['mmsi'],'name':neighbor['name'],
                           'distance_nm':metric['closest_distance_nm'],'time_s':metric['closest_time_s']})
    nearby.sort(key=lambda r:r['distance_nm'])
    alt['nearby']=nearby[:10];alt['nearby_under_05nm']=sum(r['distance_nm']<.5 for r in nearby)
    result['model']=MODEL
    result['change'].update(turn_seconds=turn_seconds,speed_seconds=speed_seconds)
    result['assumptions']=[snapshot['assumptions'][0],
