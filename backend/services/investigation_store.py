...
class Repository:
    def __init__(self, store=None):
        ...
        with self.store.transaction() as db:
            db.execute(
                "CREATE TABLE IF NOT EXISTS investigations(id TEXT PRIMARY KEY, pid TEXT NOT NULL, status TEXT NOT NULL, record TEXT NOT NULL)"
            )
            db.execute(
                "CREATE UNIQUE INDEX IF NOT EXISTS investigation_active ON investigations(pid) WHERE status IN ('queued','running')"
            )

    def _save(self, db, record):
        db.execute(
            "INSERT INTO investigations VALUES(?,?,?,?) ON CONFLICT(id) DO UPDATE SET status=excluded.status,record=excluded.record",
            (
                record["id"],
                record["proposal_id"],
                record["status"],
    ...
    def recover(self, db):
        for (raw,) in db.execute("SELECT record FROM investigations WHERE status IN ('queued','running')").fetchall():
            ...
            if not alive or time.time() > r["deadline"] + 5:
                r.update(
                    status="interrupted",
                    message="서버 재시작 또는 실행 시간 초과로 조사가 중단됐습니다. 다시 조사하세요.",
                )
                self._save(db, r)
    ...
    def create(self, pid, timeout, model):
        with self.store.transaction() as db:
            ...
            self.recover(db)
            row = db.execute(
                "SELECT record FROM investigations WHERE pid=? AND status IN ('queued','running')", (pid,)
            ).fetchone()
            if row:
            ...
            r = dict(
                ...
                model=model,
                created_at=watch_officer.iso(now),
        ...
        with self.store.transaction() as db:
            ...
            return [
                ...
                for row in db.execute(
                    "SELECT record FROM investigations WHERE pid=? ORDER BY rowid DESC LIMIT 10", (pid,)
                )
            ]
    ...
    def view(self, record):
        ...
        eligible = (
            ...
            and result.get("action") == "focus_tracking"
            and result.get("validation", {}).get("passed")
        )
    ...
    def event(self, rid, name, state, payload=None):
        ...
        if payload is not None:
            evidence[eid] = dict(
                ...
                pair=r["pair"],
                recorded_at=watch_officer.iso(time.time()),
                data=payload,
            )
...
def approved_investigation(db, rid, proposal, now, outcome="approved"):
    ...
    if (
        ...
        or r["pair"] != proposal["pair"]
        or r["status"] != "completed"
        or result.get("action") != "focus_tracking"
