...
def observe(pair, vessels, now, rule):
    ...
    high = (metric['tcpa_min'] is not None and 0 < metric['tcpa_min'] < rule['tcpa_min']
            and metric['dcpa_nm'] is not None and metric['dcpa_nm'] < rule['dcpa_nm'])
    return {
        ...
        'dcpa_nm': metric['dcpa_nm'], 'tcpa_min': metric['tcpa_min'],
...
def residuals(run, observations):
    ...
    for observation in observations:
        for v in observation['subjects']:
            ...
            points[str(v['mmsi'])].append({'t_s': elapsed, 'lat': v['lat'], 'lng': v['lng'],
                                         'x_nm': x, 'y_nm': y, 'errors_nm': errors})
    return {'tracks': points, 'note': '로컬 AIS 수신 시각 기준 위치 차이입니다. 관측 시각 오차가 포함되며 변경안의 실제 실행 여부를 뜻하지 않습니다.'}
