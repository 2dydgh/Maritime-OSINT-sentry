...
def rank_key(trigger):
    ...
    return (
        trigger.get("source") != "ml",
        float(trigger.get("dcpa_nm", math.inf)),
        float(trigger.get("tcpa_min", math.inf)),
    )
        ...
        for r in risks[source]:
            try:
                ...
                if pair[0] == pair[1] or not (math.isfinite(d) and math.isfinite(t) and d >= 0 and t > 0):
                    continue
                high = (
                    int(r.get("risk_level", 0)) >= 3
                    if source == "ml"
                    else d < config.WATCH_DCPA_NM and t < config.WATCH_TCPA_MIN
                )
                if high:
                    result[pair] = {
                        "source": source,
...
def validate(pair, risks, lookup, now, candidate):
    ...
    for v in vessels:
        ...
        if not (-90 <= v["lat"] <= 90 and -180 <= v["lng"] <= 180 and 0 <= v["sog"] < 102.3 and 0 <= v["cog"] < 360):
            return "vessel_invalid", None
    a, b = vessels
    tcpa, dcpa = collision_analyzer._compute_tcpa_dcpa(
        *(a[k] for k in ("lat", "lng", "sog", "cog")), *(b[k] for k in ("lat", "lng", "sog", "cog"))
    )
    ...
    return None, {
        "checked_at": iso(now),
        "analysis_at": candidate["analysis_at"],
        "dcpa_nm": dcpa,
        "tcpa_min": tcpa,
...
class Store:
    ...
    def transaction(self):
        ...
        try:
            ...
            db.execute(
                "CREATE TABLE IF NOT EXISTS proposals(id TEXT PRIMARY KEY, pair TEXT NOT NULL, status TEXT NOT NULL, updated REAL NOT NULL, record TEXT NOT NULL)"
            )
            ...
            db.execute(
                "CREATE TABLE IF NOT EXISTS observations(pid TEXT NOT NULL, received_key TEXT NOT NULL, at REAL NOT NULL, record TEXT NOT NULL, PRIMARY KEY(pid,received_key))"
            )
            db.execute("CREATE INDEX IF NOT EXISTS observations_time ON observations(pid,at)")
            db.execute(
                "CREATE TABLE IF NOT EXISTS handoffs(id TEXT PRIMARY KEY, at REAL NOT NULL, record TEXT NOT NULL)"
            )
            db.execute(
    ...
    def save(self, db, p, now):
        ...
        p["revision"] += 1
        db.execute(
            "INSERT INTO proposals VALUES(?,?,?,?,?) ON CONFLICT(id) DO UPDATE SET status=excluded.status,updated=excluded.updated,record=excluded.record",
            (
                p["id"],
                ":".join(map(str, p["pair"])),
                p["status"],
    ...
    def refresh(self, risks, lookup, now=None):
        ...
        all_pairs = {
            tuple(sorted((int(r["ship_a"]["mmsi"]), int(r["ship_b"]["mmsi"])))) for r in risks["ml"] + risks["distance"]
        }
        with self.transaction() as db:
            self.collect_observations(db, lookup, now)
            active = [
                json.loads(r[0])
                for r in db.execute("SELECT record FROM proposals WHERE status IN ('open','approved','tracking')")
            ]
            for p in active:
                pair = tuple(p["pair"])
                cand = cands.get(pair)
            ...
            for pair, cand in ordered:
                ...
                previous = db.execute(
                    "SELECT status,updated FROM proposals WHERE pair=? ORDER BY updated DESC LIMIT 1", (key,)
                ).fetchone()
                if previous:
                    ...
                    cooldown = (
                        config.WATCH_COOLDOWN_MIN * 60
                        if previous[0] in OPERATOR_SETTLED
                ...
                p = {
                    ...
                    "execution": None,
                    "rule": {
                        "version": "collision-watch-v2",
                        "dcpa_nm": config.WATCH_DCPA_NM,
                        "tcpa_min": config.WATCH_TCPA_MIN,
                        "max_analysis_age_sec": config.WATCH_MAX_ANALYSIS_AGE_SEC,
                        "max_vessel_age_sec": config.WATCH_MAX_VESSEL_AGE_SEC,
                    },
    ...
    def acquire_scanner(self, owner, now=None):
        ...
        with self.transaction() as db:
            ...
            if row and row[0] != owner and row[1] > now:
                return False, row[0]
            db.execute(
                "INSERT INTO leases VALUES('scanner',?,?) ON CONFLICT(name) DO UPDATE SET owner=excluded.owner, until=excluded.until",
    ...
    def record_handoff(self, operator, note, client_id, now=None):
        ...
        with self.transaction() as db:
            rows = db.execute(
                "SELECT id,status FROM proposals WHERE status IN ('open','approved','tracking')"
            ).fetchall()
            h = {
                "id": str(uuid4()),
                "at": iso(now),
    ...
    def handoff_summary(self, now=None):
        ...
        with self.transaction() as db:
            ...
            rows = db.execute(
                "SELECT record FROM proposals WHERE status IN ('open','approved','tracking') OR updated >= ?", (since,)
            ).fetchall()
    ...
    def list(self, limit=100):
        with self.transaction() as db:
            rows = db.execute(
                "SELECT p.record, (SELECT o.record FROM observations o WHERE o.pid=p.id ORDER BY o.at DESC LIMIT 1), (SELECT m.until_at FROM monitors m WHERE m.pid=p.id) FROM proposals p ORDER BY CASE WHEN status IN ('open','approved','tracking') THEN 0 ELSE 1 END,updated DESC LIMIT ?",
                (limit,),
            )
            result = []
    ...
    def decide(self, pid, outcome, reason, client_id, risks, lookup, now=None, investigation_id=None):
        ...
        with self.transaction() as db:
            ...
            if outcome == "approved":
                error, evidence = validate(
                    tuple(p["pair"]), risks, lookup, now, candidates(risks).get(tuple(p["pair"]))
                )
                if error:
            ...
            p["decision"] = {
                "scenario_review_ids": [r["id"] for r in p.get("scenario_reviews", [])],
                "at": iso(now),
                "by": "operator",
                "client_id": client_id,
                "outcome": outcome,
    ...
    def monitor(self, db, pid, until):
        db.execute(
            "INSERT INTO monitors VALUES(?,?) ON CONFLICT(pid) DO UPDATE SET until_at=MAX(until_at,excluded.until_at)",
            (pid, until),
        )

    def collect_observations(self, db, lookup, now):
        rows = db.execute(
            "SELECT p.record FROM proposals p JOIN monitors m ON p.id=m.pid WHERE m.until_at>=?", (now,)
        ).fetchall()
        for row in rows:
            p = json.loads(row[0])
            try:
            ...
            db.execute(
                "DELETE FROM observations WHERE pid=? AND received_key NOT IN (SELECT received_key FROM observations WHERE pid=? ORDER BY at DESC LIMIT 1200)",
                (p["id"], p["id"]),
            )
            ...
            reviews.append(
                {
                    ...
                    "outcome": outcome,
                    "reason": reason,
                    "snapshot_at": run["snapshot"]["created_at"],
                    "change": run["change"],
                    "model": run["model"],
    ...
    def followup(self, pid, now=None):
        ...
        return {
            "proposal_id": pid,
            "state": state,
            "until": iso(monitor[0]) if monitor else None,
            "latest": latest,
            "observations": observations,
            "approval_evidence": p.get("approval_evidence"),
