...
def valid(v, now):
    return (
        all(
            isinstance(v.get(k), (int, float)) and not isinstance(v[k], bool) and math.isfinite(v[k])
            for k in ("lat", "lng", "sog", "cog", "_updated")
        )
        and abs(v["lat"]) <= 80
        and -180 <= v["lng"] <= 180
        and 0 <= v["sog"] < 102.3
        and 0 <= v["cog"] < 360
        and 0 <= now - v["_updated"] <= MAX_AGE
    )
...
def capture(pair, vessels=None, now=None):
    ...
    excluded = 0
    for v in vessels:
        if int(v["mmsi"]) in pair:
            continue
        try:
            distances = [math.hypot(*xy(v["lat"], v["lng"], s)) for s in selected]
            if not math.isfinite(min(distances)) or min(distances) > CONTEXT_NM:
                continue
    ...
    for v in selected + [v for _, v in context[:MAX_NEIGHBORS]]:
        x, y = xy(v["lat"], v["lng"], origin)
        vx, vy = velocity(v)
        age = now - v["_updated"]
        ships.append(
            {
                "mmsi": int(v["mmsi"]),
                "name": v.get("name") or str(v["mmsi"]),
                "type": v.get("type", "other"),
                "sog": v["sog"],
                "cog": v["cog"],
                "received_at": stamp(v["_updated"]),
                "source_lat": v["lat"],
                "source_lng": v["lng"],
                "x_nm": x + vx * age,
                "y_nm": y + vy * age,
    ...
    return {
        ...
        "ships": ships,
        "coverage": {
            "radius_nm": CONTEXT_NM,
            "included": len(ships) - 2,
            "stale_or_invalid": excluded,
            "capacity_omitted": omitted,
            "assessment": "partial",
            "note": "수신된 AIS 중 주변 10해리·최대 50척만 확인합니다. 미수신 선박과 범위 밖에서 진입하는 선박은 평가하지 못합니다.",
        },
        "assumptions": [
            "로컬 수신 시각을 기준으로 동일 시점까지 등속 투영합니다. 실제 센서 관측 시각과 다를 수 있습니다.",
...
def closest(a, b, horizon, origin):
    ...
    return {
        ...
        "closest_time_s": bounded,
        "tcpa_min": t / 60 if t is not None else None,
        "dcpa_nm": math.hypot(rx + vx * t, ry + vy * t) if t is not None and t >= 0 else None,
        "within_horizon": t is not None and 0 <= t <= horizon,
        "a": point(a, bounded, origin),
        "b": point(b, bounded, origin),
    }


def compare(snapshot, target, speed, course, minutes, turn_seconds=0, speed_seconds=0):
    if snapshot.get("model") != MODEL:
        raise ValueError("지원하지 않는 스냅샷 모델입니다.")
    if target not in snapshot["pair"]:
        raise ValueError("비교 대상 두 척 중 변경할 선박을 선택하세요.")
    if not all(math.isfinite(v) for v in (speed, course, minutes)) or not (
        0 <= speed <= 50 and 0 <= course < 360 and 5 <= minutes <= 30
    ):
    ...
    horizon = minutes * 60
    steps = list(range(0, int(horizon), 10)) + [horizon]
    result = {
        "id": str(uuid4()),
        "created_at": stamp(time.time()),
        "snapshot": snapshot,
        "model": MODEL,
        "change": {"mmsi": target, "sog": speed, "cog": course},
        "horizon_s": horizon,
        "scenarios": {},
    }
    for key in ("baseline", "alternative"):
        ...
        if key == "alternative":
            for v in ships:
                if v["mmsi"] == target:
                    v.update(sog=speed, cog=course)
        by_id = {s["mmsi"]: s for s in ships}
        a, b = [by_id[m] for m in snapshot["pair"]]
        nearby = []
        for subject in (a, b):
        ...
        result["scenarios"][key] = {
            "pair": closest(a, b, horizon, snapshot["origin"]),
            "nearby": nearby[:10],
            "nearby_under_05nm": sum(r["distance_nm"] < 0.5 for r in nearby),
            "tracks": {
                str(v["mmsi"]): {"ship": v, "points": [point(v, t, snapshot["origin"]) for t in steps]} for v in ships
            },
        }
...
class Repository:
    def __init__(self, path=None):
        self.path = Path(path or config.SCENARIO_DB)
        ...
        try:
            with db:
                db.execute(
                    ...
                    (record["id"], kind, record["created_at"], json.dumps(record, ensure_ascii=False, allow_nan=False)),
                )
        finally:
            db.close()
        return record
    ...
    def get(self, kind, rid):
        db = self.connect()
        try:
            row = db.execute("SELECT body FROM records WHERE kind=? AND id=?", (kind, rid)).fetchone()
        finally:
            db.close()
        if not row:
            raise KeyError(rid)
    ...
    def recent(self):
        ...
        try:
            try:
                rows = db.execute(self._RECENT_SQL).fetchall()
        ...
        for rid, created, name_a, name_b, mmsi_a, mmsi_b, changed in rows:
            ...
            changed_name = (
                name_a
                if changed is not None and changed == mmsi_a
                else name_b
                if changed is not None and changed == mmsi_b
                else None
            )
