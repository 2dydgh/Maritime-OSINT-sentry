...
QUESTIONS = {
    "why": "왜 추적 대상으로 판단했나?",
    "basis": "판단 근거는 무엇인가?",
    "after": "판단 이후 무엇이 달라졌나?",
}
STATUSES = {
    "open": "검토 대기",
    "approved": "승인됨·실행 확인 대기",
    "tracking": "추적 중",
    "dismissed": "기각",
    "expired": "만료",
    "failed": "실행 실패",
    "unknown": "실행·관측 상태 확인 불가",
    "completed": "종료",
}
...
def load(pid, watch_path=None, scenario_path=None):
    ...
    try:
        ...
        if "observations" in tables:
            ...
            rows = db.execute(
                "SELECT record FROM observations WHERE pid=? ORDER BY at DESC,received_key LIMIT ?",
                (pid, MAX_OBSERVATIONS),
            ).fetchall()
...
class Evidence:
    ...
    def node(self, kind, label, raw, pointer, path=None, identifier=None):
        node = identifier or URIRef(
            "urn:mos:" + kind.lower() + ":" + digest([self.pid, path or self.path, pointer, raw])
        )
    ...
    def state(self, v, pointer, path=None, received=None, source_raw=None):
        ...
        node = self.node(
            kind,
            ("수신 관측" if received is not None else "수신 시각 없는 과거 입력") + " · " + str(v.get("mmsi", "")),
            v if source_raw is None else source_raw,
            pointer,
            path,
        )
    ...
    def analysis(self, raw, pointer, label, approval=False):
        ...
        for i, v in enumerate(raw.get("subjects", [])):
            obs, kind = self.state(
                v, pointer + "/subjects/" + str(i), received=received[i] if i < len(received) else None
            )
    ...
    def scenario(self, run):
        ...
        for i, v in enumerate(snap.get("ships", [])[:2]):
            ...
            projection = self.node(
                "ProjectedState", "비교 시작 상태 · " + str(v.get("mmsi", "")), v, "/snapshot/ships/" + str(i), path
            )
    ...
    def build(self):
        ...
        for mmsi in p.get("pair", []):
            v = next(
                (v for v in p.get("trigger", {}).get("subjects", []) if str(v.get("mmsi")) == str(mmsi)), {"mmsi": mmsi}
            )
        ...
        for i, review in enumerate(p.get("scenario_reviews", [])[:50]):
            node = self.node(
                "Review",
                "검토 · "
                + {"reference": "참고안", "deferred": "보류", "rejected": "미채택"}.get(
                    review.get("outcome"), "결과 미기록"
                ),
                review,
                "/scenario_reviews/" + str(i),
            )
            ...
            if (
                run.get("id") != rid
                or run.get("snapshot", {}).get("proposal_id") != self.pid
                or sorted(run["snapshot"].get("pair", [])) != sorted(p.get("pair", []))
            ):
        ...
        if p.get("decision"):
            ...
            if raw.get("outcome") == "approved" and p.get("approval_evidence"):
                self.link(
                    decision,
                    "basedOn",
                    self.analysis(p["approval_evidence"], "/approval_evidence", "승인 시 CPA 재검증", approval=True),
                )
        ...
        if execution:
            ...
            for i, event in enumerate(p.get("events", [])):
                is_receipt = event.get("kind") == "execution_receipt" or event.get("reason") in (
                    "map_tracking_started",
                    "operator_stopped_tracking",
                    "map_tracking_stopped",
                )
                if not is_receipt or event.get("status") not in ("tracking", "completed", "failed"):
                    continue
                receipt = self.node(
                    "ExecutionReceipt",
                    "브라우저 영수증 · " + STATUSES.get(event["status"], event["status"]),
                    event,
                    "/events/" + str(i),
                )
        ...
        for observation in self.bundle.get("observations", []):
            ...
            node = self.node(
                "FollowupAssessment", "후속 거리·CPA 평가", observation, "/observations/by-content/" + key, path
            )
        ...
        if total > MAX_OBSERVATIONS:
            self.gap(
                f"근거 그래프에는 저장된 후속 평가 {total}건 중 최신 {MAX_OBSERVATIONS}건을 포함했습니다. 전체 기록은 후속 확인에서 볼 수 있습니다."
            )
    ...
    def validation(self):
        conforms, report, _ = validate(
            self.g,
            shacl_graph=definition("shapes.ttl"),
            inference="none",
            advanced=False,
            js=False,
            ...
            violations.append(
                {
                    ...
                    "message": str(report.value(result, SH.resultMessage) or "데이터 제약 위반"),
                }
            )
    ...
    def answer(self, question):
        ...
        if question == "why":
            ...
            claim(
                ...
                + "”입니다. 현재 브라우저의 실제 추적 상태와 다를 수 있습니다.",
                self.root,
            )
            if row and row.decision:
                outcome = str(row.outcome)
                claim(
                    ("집중 추적을 승인했습니다." if outcome == "approved" else "제안을 기각했습니다.")
                    + " 기록된 판단 이유: "
                    + str(row.reason or "기록 없음"),
                    row.decision,
                )
                if row.analysis and row.dcpa is not None:
                    claim(
                        "승인 시 재검증 DCPA는 "
                        + format(float(row.dcpa), ".2f")
                        + " nm"
                        + (" · TCPA " + format(float(row.tcpa), ".1f") + "분" if row.tcpa is not None else "")
                        + "입니다.",
                        row.analysis,
                    )
                ...
                if receipts:
                    claim(
                        "브라우저가 지도 추적 시작을 확인한 과거 영수증이 있습니다. 선박 운항 변경의 실행 기록은 아닙니다.",
                        receipts[0],
                    )
            ...
            claim(
                "제안에 저장된 분석과 원본 입력을 연결했습니다. 수신 시각이 빠진 입력은 관측과 구분해 표시합니다.",
                self.g.value(self.root, M.triggeredBy),
            )
            if decision:
                claim(
                    "확인 가능한 판단 당시 검토 연결은 "
                    + str(len(reviews))
                    + "건입니다. 이후에 추가된 검토는 이 목록에 포함하지 않습니다.",
                    decision,
                    *reviews,
                )
            else:
            ...
            if not decision_at:
                ...
                fresh = bool(
                    until
                    and self.now <= until.toPython().timestamp()
                    and len(stamps) == 2
                    and all(t and 0 <= self.now - t.toPython().timestamp() <= 60 for t in stamps)
                )
                claim(
                    ("최근 유효한 수신 기록" if fresh else "과거 수신 기록")
                    + "의 선박 간 거리는 "
                    + format(float(latest.distance), ".2f")
                    + " nm입니다.",
                    latest.node,
                )
                if not fresh:
                    ...
                    claim(
                        ...
                        + (
                            "유지됩니다."
                            if str(latest.state) == "high"
                            else "충족되지 않습니다. 안전이 확정됐다는 뜻은 아닙니다."
                        ),
                        latest.node,
                    )
                if len(rows) > 1:
                    ...
                    claim(
                        "표시 범위에서 관측 거리는 "
                        + format(float(first.distance), ".2f")
                        + " → "
                        + format(float(latest.distance), ".2f")
                        + " nm로 변했습니다. 추적 조치가 이 변화를 일으켰다는 뜻은 아닙니다.",
                        first.node,
                        latest.node,
                    )
        if not validation["conforms"]:
            claims = [
                {
                    "text": "원본 연결 또는 데이터 형식이 검증 조건을 충족하지 않아 결론을 제공하지 않습니다. 아래 검증 결과와 원본을 확인하세요.",
                    "evidence": [],
                }
            ]
        ...
        for nid, meta in self.nodes.items():
            ...
            for predicate, value in sorted(self.g.predicate_objects(node), key=lambda pair: str(pair[0])):
                if isinstance(value, Literal) and predicate not in (RDFS.label, M.sourcePath, M.sourcePointer):
                    properties.append(
                        {
                            "label": str(ontology.value(predicate, RDFS.label) or predicate.split("#")[-1]),
                            "value": str(value),
                        }
                    )
            nodes.append(
                dict(
                    ...
                    source_url="/api/v1/knowledge/proposals/"
                    + quote(self.pid, safe="")
                    + "/source?node="
                    + quote(nid, safe="")
                    + "&fingerprint="
                    + digest(self.sources[nid]["record"]),
                    relevant=nid in relevant,
                    run_id=str(self.g.value(node, M.runId)) if self.g.value(node, M.runId) else None,
                )
            )
        edges = [
            {
                ...
                "to": str(b),
                "relation": str(rel),
                "label": str(ontology.value(rel, RDFS.label) or rel.split("#")[-1]),
            }
            for a, rel, b in self.g
            if isinstance(b, URIRef) and str(a) in self.nodes and str(b) in self.nodes
        ]
        return {
            "proposal_id": self.pid,
            "revision": self.p.get("revision"),
            "ontology_version": VERSION,
            "generated_at": datetime.fromtimestamp(self.now, UTC).isoformat(),
            "question": question,
            "question_label": QUESTIONS[question],
            "claims": claims,
            "gaps": self.gaps,
            "validation": validation,
            "graph": {"nodes": nodes, "edges": sorted(edges, key=lambda e: (e["from"], e["relation"], e["to"]))},
            "coverage": {
                "followup_included": len(self.bundle.get("observations", [])),
                "followup_total": self.bundle.get("observation_total", 0),
            },
